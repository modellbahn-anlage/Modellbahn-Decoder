...
//            2013-06-04 V0.13 added compile switch for traffic light (removed)
//                             added AspectTarget to get correct saving
//            2014-09-19 V0.14 saving hauptsignal_state and vorsignal_state
//
//------------------------------------------------------------------------
//
...
#define BELGIAN_SIGNALS    0     // 0: standard mode
                                 // 1: only Belgian signals

#define DUTCH_SIGNALS      0     // 0: standard mode
                                 // 1: only Dutch signals
#define TRAFFIC_LIGHTS     0     // 0: no OpMode 7 (traffic lights)
                                 // 1: OpMode 7 included
typedef union
{
    uint16_t as_uint16;
...
#if (F_CPU == 10000000UL)
    const unsigned char speed[] PROGMEM = {"..10MHz.."};
#endif
#if (F_CPU == 8000000UL)
    const unsigned char speed[] PROGMEM = {"..8MHz.."};
#endif
...
unsigned char EE_hauptsignal_state  EEMEM = 0;    // aktueller Zustand Hauptsignal
unsigned char EE_vorsignal_state    EEMEM = 0;    // aktueller Zustand Vorsignal
